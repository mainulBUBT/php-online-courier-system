...
if(isset($_POST['update']))
{
  ...
  if ($conn->query($sql) === TRUE) {
    ...
    <script type="text/javascript">
                ...
                window.location = "pending_parcel.php";
                </script>
                <?php
} else {
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post"  action="pending_parcel.php">
...
<div class="form-group">
 ...
 //------------------------- Condition for Single Value Search --------------------//
  ...
  $sql = "SELECT `del_area`, `recv_name`, `recv_address`,`del_man`,`par_status`,`p_id` FROM delivery_info NATURAL JOIN parcel where parcel.par_status='Not pickup yet' and p_id LIKE '%$track_id%'";
$result = $conn->query($sql);
?>
...
else
{
  ...
  $sql = "SELECT `del_area`, `recv_name`, `recv_address`,`del_man`,`par_status`,`p_id` FROM delivery_info NATURAL JOIN parcel where parcel.par_status='Not pickup yet' ORDER BY p_id ASC limit $start_form, $num_per_page";
...
<?php 
      $sql = "SELECT `del_area`, `recv_name`, `recv_address`,`del_man`,`par_status`,`p_id` FROM delivery_info NATURAL JOIN parcel where parcel.par_status='Not pickup yet'";
    $result = $conn->query($sql);
    $totalrecord = $result->num_rows;
    $totalpages = ceil($totalrecord/$num_per_page);
    for ($i=1; $i <=$totalpages ; $i++) { 
      echo "<a href='pending_parcel.php?page=".$i."' class='btn btn-own text-light' style='margin-left: 10px; margin-top: 10px'>$i</a>";
    }
